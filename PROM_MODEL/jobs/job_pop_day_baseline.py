...
from dependencies.algorithm.pop.sale_pop_baseline_t0 import SalePopBaselineForecast
...
def transform_data(spark: SparkSession, raw_data_df: DataFrame, config: dict) -> (DataFrame, dict):
    ...
    sale_pop_baseline_forecast = SalePopBaselineForecast(config["current_date"])
    ...
    preprocessed_rdd = raw_data_df.rdd.map(lambda row: ((row['wh_id']), list(row))) \
        ...
        .flatMap(lambda item: sale_pop_baseline_forecast.preprocess_lr_data(item, config))\
        .reduceByKey(lambda x, y: x+y).persist()

    fu.log_print("LR 阶段开始...")
    lr_forecast = train_n_predict(spark, preprocessed_rdd, config)
    fu.log_print("LR 阶段结束！LT 阶段开始...")
    lt_forecast = preprocess_lt_date(spark, preprocessed_rdd, lr_forecast, config)
    fu.log_print("LT 阶段结束!")
    final_rst = lr_forecast.select(*config['pred_cols']).union(
...
# helper
#############################################
def train_n_predict(spark, preprocessed_rdd, config):
    df_train_rdd = preprocessed_rdd.filter(lambda x: x[0] == 'df_train').values().reduce(lambda x, y: x+y)
    df_train = pd.DataFrame.from_records(df_train_rdd)
    df_test_rdd = preprocessed_rdd.filter(lambda x: x[0] == 'df_test').values().reduce(lambda x, y: x+y)
    df_test = pd.DataFrame.from_records(df_test_rdd)
    ...
    # 模型训练
    model = SalePopBaselineForecast.train_lr_model(df_train[config['ordered_cols']].values)

    # 模型预测
    lr_forecast = SalePopBaselineForecast.lr_predict(model, df_test, config['ordered_cols'])
    return spark.createDataFrame(lr_forecast)


def preprocess_lt_date(spark, preprocessed_rdd, lr_forecast, config):
    train_data = preprocessed_rdd.filter(lambda x: x[0] == 'train_data').flatMap(lambda x: x[1]).map(
        lambda x: Row(**x)).toDF().repartition(100, 'wh_id')
    longtail_data = preprocessed_rdd.filter(lambda x: x[0] == 'longtail_data').flatMap(lambda x: x[1]).map(
        lambda x: Row(**x)).toDF().repartition(100, 'wh_id')
    raw_data = preprocessed_rdd.filter(lambda x: x[0] == 'raw_data').flatMap(lambda x: x[1]).map(
        lambda x: Row(**x)).toDF().repartition(100, 'wh_id')

    # LR未预测SKU
    tr_skus = train_data.dropDuplicates(subset=['wh_id', 'bu_id', 'sku_id']).select('wh_id', 'bu_id', 'sku_id')
    lr_skus = lr_forecast.dropDuplicates(subset=['wh_id', 'bu_id', 'sku_id']).select('wh_id', 'bu_id', 'sku_id')
    lr_skus = lr_skus.withColumn('lr', F.lit(1))

    merged = tr_skus.join(lr_skus, how='left', on=['wh_id', 'bu_id', 'sku_id'])
    ...
    cnt_longtail = raw_data.filter(F.col('is_train') == 1).join(lt_skus, how='inner', on=lt_skus.schema.names)\
    ...
    cnt_avg_rdd = cnt_longtail.rdd.map(lambda row: ((row['wh_id']), list(row))) \
        .aggregateByKey(list(), seqOp, combOp) \
        .sortByKey(numPartitions=100, keyfunc=lambda x: x) \
        .flatMap(lambda item: SalePopBaselineForecast.get_avg_result(item, config)).persist()
    ...
    lt_forecast = lt_data.rdd.map(lambda row: ((row['wh_id']), list(row))) \
        .aggregateByKey(list(), seqOp, combOp) \
        .sortByKey(numPartitions=100, keyfunc=lambda x: x) \
        .flatMap(lambda item: SalePopBaselineForecast.gen_lt_prediction(item, config)).persist()
